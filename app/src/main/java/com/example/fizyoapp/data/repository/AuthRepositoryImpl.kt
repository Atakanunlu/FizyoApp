...
class AuthRepositoryImpl @Inject constructor() : AuthRepository {
    ...
    override fun signIn(
        ...
        try {
            ...
            val userId = result.user?.uid ?: "***Kullanıcı ID bulunamadı.***"
            ...
            // Başaralı oldugunda giriş yapıcak.
    ...
    override fun signUp(
        ...
        try {
            ...
            try {
                ...
                //Kayıt oluktan sonra oturumu kapattım ki kullanıcı kendisi login ekrandan girsin
                auth.signOut()
            ...
            CoroutineScope(Dispatchers.IO).launch {
                if (user != null) {
                    try {
                        ...
                        val adminDoc = physiotherapistCollection.document(user.uid).get().await()
                        if (adminDoc.exists()) {
                            val userObj = User(
                                id = user.uid,
                                email = user.email ?: "",
