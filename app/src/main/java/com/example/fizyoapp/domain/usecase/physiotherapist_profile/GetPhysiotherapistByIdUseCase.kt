...
class GetPhysiotherapistByIdUseCase @Inject constructor(
    private val repository: PhysiotherapistProfileRepository
) {
    operator fun invoke(physiotherapistId: String): Flow<Resource<PhysiotherapistProfile>> {
        return repository.getPhysiotherapistById(physiotherapistId)
    }
}